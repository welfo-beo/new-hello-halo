...
function formatTraceResults(duration: number, metrics: Record<string, number>): string {
  const lines = [
    ...
    '## Trace Summary', `Duration: ${duration}ms`, '', '## Core Metrics'
  ]
...
const browser_perf_start = tool(
  ...
  {
    reload: z.boolean().describe('Determines if the page should be automatically reloaded.'),
    autoStop: z.boolean().describe('Determines if the trace should be automatically stopped.')
  },
  async (args) => {
    const viewId = browserContext.getActiveViewId()
    if (!viewId) return textResult('No active browser page.', true)
    if (browserContext.isPerformanceTracing()) {
      return textResult('Error: a performance trace is already running. Use browser_perf_stop to stop it.', true)
    }
    try {
      if (args.reload) {
...
const browser_perf_stop = tool(
  ...
  {},
  async () => {
    if (!browserContext.getActiveViewId()) return textResult('No active browser page.', true)
    if (!browserContext.isPerformanceTracing()) return textResult('No performance trace is running.')
    try {
      const { duration, metrics } = await browserContext.stopPerformanceTrace()
      return textResult(formatTraceResults(duration, metrics))
    } catch (error) {
...
const browser_perf_insight = tool(
  ...
  {
    insightSetId: z.string().describe('The id for the specific insight set.'),
    insightName: z.string().describe('The name of the Insight (e.g. "DocumentLatency", "LCPBreakdown")')
  },
  async (args) => {
    if (!browserContext.getActiveViewId()) return textResult('No active browser page.', true)
    try {
...
export const performanceTools = [browser_perf_start, browser_perf_stop, browser_perf_insight]
